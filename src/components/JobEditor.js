...
export default function JobEditor({ jobToEdit, props }) {
  ...
  return (
    <div className="w-full sm:w-3/4 md:w-1/2 lg:w-2/5 xl:w-1/3 mx-auto gap-4 mt-10 grid">
      <div className="flex justify-end gap-2 mb-4">
        ...
        >
          <FontAwesomeIcon
            icon={faPen}
            className={`${editing ? "text-money" : "text-gray-400"} mr-2.5 text-sm`}
          />
          Edit
        </button>
        <button
      ...
      {editing ? (
        <div className="bg-slate-100 shadow-lg px-6 py-8 rounded-xl">
          <div>
            <InputField
              className="mt-8"
              type="text"
            ...
            <InputField
              ...
              value={jobs.jobType}
              onChange={(e) => setJobs({ ...jobs, jobType: e.target.value })}
            />
            <InputField
              className="mt-6"
              type="text"
              ...
              value={jobs.duration}
              onChange={(e) => setJobs({ ...jobs, duration: e.target.value })}
            />
            <InputField
              className="mt-6"
              type="text"
            ...
            <InputField
              ...
              value={jobs.deadline}
              onChange={(e) => setJobs({ ...jobs, deadline: e.target.value })}
            />
            <InputField
              className="mt-6"
              type="number"
              ...
              onChange={(e) => setJobs({ ...jobs, maxApplicants: e.target.value })}
            />
            <InputField
              className="mt-6"
              type="number"
              ...
              onChange={(e) => setJobs({ ...jobs, maxPositions: e.target.value })}
            />
            <div className="mt-6 mb-4">
              <label className="block text-sm font-semibold mb-2">Skills <span className="text-red-500">*</span></label>
              <MuiChipsInput
                value={tags}
                onChange={(e) => addTag(e)}
                className="bg-white w-full border p-3 rounded-lg mb-4"
                onDeleteChip={(deletedTag) => handleDeleteTag(deletedTag)}
              />
            </div>
            <label className="block text-sm font-medium text-gray-700 mt-6 mb-2">Job Description</label>
            <ReactQuill
              modules={modules}
              theme="snow"
              value={jobs.description}
              onChange={(e) => setJobs({ ...jobs, description: e })}
      ...
      <div className="flex flex-col sm:flex-row items-center mt-12 gap-4">
        {isComplete ? (
          ...
          <Button variant="contained" color="secondary" disabled className="w-full sm:w-auto">
            Waiting...
          </Button>
        )}

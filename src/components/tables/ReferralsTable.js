...
export default function ReferralsTable(props) {
  ...
  const sortedData = referrals.sort((a, b) => {
    if (orderBy) {
    ...
    <>
        ...
        <TableContainer>
          <Table className="min-w-full divide-y divide-gray-500 z-0">
            <TableHead>
              <TableRow>
                {th.map((t, index) => (
                  <TableCell key={index} className="px-6 py-3 text-left text-xs text-gray-900 uppercase tracking-wider font-semibold">
                    <TableSortLabel
                      active={orderBy === t}
                      direction={orderBy === t ? order : 'asc'}
                      onClick={() => handleRequestSort(t)}
                    >
                      {t}
                    </TableSortLabel>
                  </TableCell>
                ))}
                <TableCell className="px-6 py-3 text-left">Actions</TableCell>
              </TableRow>
            </TableHead>
      ...
      <div className="mt-10">
        <span className="font-normal text-slate-500">
          ...
          <span className="font-semibold uppercase text-blue-500">accepted</span>, click on
          the <span className="font-semibold uppercase text-blue-500">'Rate Job'</span> line
          to evaluate it.
      ...
      {/* <Modal
        show={open}
        onClose={handleClose}
        className={`transition-opacity ${open ? "opacity-100 transition duration-300" : "opacity-0"
          } bg-overlay-70`}
        size="md"
      >
          ...
          <div className="min-h-screen px-4 text-center">
            ...
            >
              <div className="inline-block w-full max-w-lg md:p-6 p-3 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                <h5 className="text-center text-xl font-semibold text-blue-500">Please rate the job based on your experience so far</h5>

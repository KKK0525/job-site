...
export default function JobAd({ job, tags, about, edit }) {
  ...
  return (
    <>
      {job && (
        <div className="w-11/12 mx-auto mt-32 pb-8">
          <div className="flex">
            {recruiter.map((recruiter, index) => (
              <img
                alt="company logo"
                className="md:h-24 md:w-24 w-32 h-32 md:mr-6 mr-4 rounded-full"
                    ...
                    {tags.map((tag, index) => (
                      <div
                        ...
                        className="relative grid select-none items-center whitespace-nowrap rounded-lg 
                          bg-blue-300 py-1.5 px-3 font-sans text-xs font-bold uppercase text-white"
                      >
      ...
      {about && (
        <div>
          {/* LEFT */}
          <div className="rounded-xl bg-gray-100 pd:pt-1 pt-20 shadow-lg">
            <div className="flex flex-row justify-self-end mr-5">
              <Link to={`/jobs`}>
                <Button className="ml-auto">Job list</Button>
              </Link>
            </div>
            <div className="w-11/12 mx-auto mt-10 pb-4">
              <div className="w-full flex items-center justify-between">
                <div className="w-3/4 flex gap-2">
                  {recruiters.map((recruiter, index) => (
                    <img
                      alt="company logo"
                      className="md:h-24 md:w-24 w-30 h-30 md:mr-6 mr-4 rounded-full"
                      src={recruiter.profile}
                      key={index}
                    />
                  ))}
              ...
              <div className="flex gap-3">
                ...
                <div className="flex flex-row-reverse gap-1">
                  {about.skillsets.slice(0, 5).map((tag, index) => (
                    <div
                      key={index}
                      className="relative grid select-none items-center whitespace-nowrap rounded-lg 
                          bg-gray-500 py-1.5 px-3 font-sans text-xs font-bold uppercase text-white"
                    >
                      <span className="">{tag}</span>
                    </div>
                  ))}
              ...
              <div className="w-full flex flex-col md:flex-row gap-2 items-center my-10">
                <div className="bg-blue-500 w-40 h-16 rounded-lg flex flex-col items-center justify-center mb-3">
                  <span className="text-md text-white">Salary</span>
                  <p className="text-2xl font-semibold text-white">
                    $ {about?.salary}
                  </p>
                </div>

                <div className="text-blue-500 w-40 h-16 border border-1 border-blue-500 rounded-lg flex flex-col items-center justify-center mb-3">
                  <span className="text-md text-blue-500">Job Type</span>
                  <p className="text-2xl font-semibold text-blue-600">
                    {about.jobType}
                  </p>
                </div>

                <div className="text-blue-500 border border-1 border-blue-500 w-40 h-16 px-6 rounded-lg flex flex-col items-center justify-center mb-3">
              ...
              {/* <div className="w-full flex justify-center items-center my-4">
                <div
                  className={`text-${about.maxPositions - about.acceptedCandidates > 0 ? 'green-500' : 'red-500'} w-42 h-16 px-6 rounded-lg flex flex-col items-center justify-center`}
                >
                  <span className="text-md">Remaining Positions</span>
                  <p className="text-2xl font-semibold text-blue-600">
